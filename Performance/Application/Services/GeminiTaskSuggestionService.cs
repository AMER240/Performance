...
namespace Performance.Application.Services
{
    ...
    public class GeminiTaskSuggestionService : ITaskSuggestionService
    {
        ...
        public GeminiTaskSuggestionService(PerformanceDbContext db, IConfiguration configuration, HttpClient httpClient)
        {
            _db = db;
        ...
        public async Task<SuggestionResult> SuggestAsync(string taskDescription, int? projectId = null)
        {
            ...
            try
            {
                ...
                // Parse response
                var suggestion = ParseGeminiResponse(geminiResponse, users);
        ...
        private async Task<string> GetHistoricalContext(int? projectId)
        {
            var completedTasks = await _db.Tasks
                .Include(t => t.Project)
                .Where(t => t.Status == Infrastructure.Entities.TaskStatus.Done)
            ...
            {
                completedTasks = completedTasks.Where(t => t.ProjectId == projectId.Value).ToList();
            }
...
{{
  ...
  ""explanation"": ""Your reasoning here""
        ...
        private SuggestionResult ParseGeminiResponse(string geminiResponse, List<UserEntity> users)
        {
            try
            {
        ...
        {
            var defaultUsers = _db.Users.Take(1).Select(u => u.Id).ToArray();
